...
def mandelbrot_colorize(image_width, image_height, max_iterations, gradient, image_data, histogram, iterations_per_pixel, smoothed_distances_to_next_iteration_per_pixel, normalized_colors):
    ...
    for pixel_y in xrange(image_height):
        for pixel_x in xrange(image_width):
            iter = iterations_per_pixel[pixel_y * image_width + pixel_x]  # 1 .. max_iterations
            ...
            if iter == max_iterations:
                ...
                smoothed_distance_to_next_iteration = smoothed_distances_to_next_iteration_per_pixel[pixel_y * image_width + pixel_x]  # 0 .. <1.0
            ...
            image_data[3 * (pixel_y * image_width + pixel_x) + 0] = int(255.0 * r)
            image_data[3 * (pixel_y * image_width + pixel_x) + 1] = int(255.0 * g)
            image_data[3 * (pixel_y * image_width + pixel_x) + 2] = int(255.0 * b)
